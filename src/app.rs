...
impl App {
    ...
    fn update_buffers(&mut self) {
        match request_and_parse() {
            Ok(all) => {
                let skip =if all.len() > BUFF_LEN { all.len() - BUFF_LEN } else { all.len() };
    ...
    fn update(&mut self, ctx: &egui::CtxRef, frame: &mut epi::Frame<'_>) {
        ...
        egui::CentralPanel::default()
            .show(ctx, |ui| {
                ...
                let curve = Curve::from_ys_f32(&self.buffs)
                    .name("co2");
...
fn make_request() -> Result<json::JsonValue, String> {
    let out_bytes = match Command::new("sh")
        .arg("-c")
        .arg(format!(r#"curl -G "{}" --data-urlencode "api_key={}""#, DB_URL, API_KEY))
        .output() {
            Ok(v) => v.stdout,
            Err(e) => {
                return Err(format!("Error running curl, check database URL and the API key: {}", e));
            },
        };
...
fn request_and_parse() -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    let mut values = vec![];
    for obj in make_request()?.entries().skip(1).next().unwrap().1.members() {
